...
class ProjectTask(models.Model):
    ...
    #     # Avoid adding recipients to followers
    #     if self._context.get('prohibit_followers'):
    #         self = self.with_context(
    #             mail_create_nolog=True,  # Disable logs
    #             mail_post_autofollow=False,  # 👈 Disable auto-follow
    #             mail_post_autofollow_partners=[],  # 👈 Empty autofollow list
    #         )
        ...
        for rec in self:
            ...
            if rec and rec.timesheet_ids:
                timesheet_line = rec.timesheet_ids.filtered(
                    lambda x: x.task_id.id == rec.id and x.end_date == False and x.start_date != False)
                if timesheet_line:
    ...
    def action_task_start(self):
        if self.task_running and not self.env.company.ts_allow_multi_user:
    ...
    def action_task_end(self):
        ...
        # Ensure `start_time` is a valid datetime and not a boolean
        if isinstance(self.env.user.start_time, datetime):
            ...
            raise UserError("Start time is not valid for the user. Cannot calculate duration.")
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'ts.task.time.account.line',
            'target': 'new',
        }
    ...
    # def open_mail_compose_no_followers(self):
    #     self.ensure_one()
    #     return {
    #         'type': 'ir.actions.act_window',
    ...
    def action_open_custom_email_wizard(self):
        return {
            ...
            'context': {
                ...
                'default_subject': f"Regarding Task: {self.name}",
            },
        }